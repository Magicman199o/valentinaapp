...
const AdminPage = () => {
  ...
  const createVIPCode = async () => {
    ...
    const existingCode = vipCodes.find(
      (c) => c.assigned_user_id === newVIPUser,
    );
    if (existingCode) {
  ...
  const createManualMatch = async () => {
    ...
    const existingMatch = matches.find(
      (m) =>
        (m.male_user_id === manualMatchMale &&
          m.female_user_id === manualMatchFemale) ||
        (m.male_user_id === manualMatchFemale &&
          m.female_user_id === manualMatchMale),
    );
    ...
    try {
      await callAdminAPI("createMatch", {
        maleUserId: manualMatchMale,
        femaleUserId: manualMatchFemale,
      });
  ...
  const updateUser = async (userId: string, updates: Record<string, any>) => {
    try {
      await callAdminAPI("updateUser", { userId, updates });
  ...
  const oppositeGenderOptions = newVIPUser
    ? getOppositeGenderUsers(newVIPUser)
    : [];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-secondary via-background to-muted">
        <motion.div
          ...
          className="card-romantic w-full max-w-md mx-4"
        >
          ...
          <form onSubmit={handleAdminLogin} className="space-y-4">
            ...
            <Button type="submit" className="w-full btn-romantic">
              Login
            </Button>
          </form>
        </motion.div>
          ...
          <Tabs defaultValue="users">
            <TabsList className="mb-4 flex-wrap">
              ...
              <TabsTrigger value="inquiries">
                ...
                Inquiries ({sponsorInquiries.length})
              </TabsTrigger>
            </TabsList>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {users.map((u) => (
                        <tr
                          key={u.id}
                          className={`border-b hover:bg-secondary/50 ${selectedUsers.includes(u.user_id) ? "bg-primary/10" : ""}`}
                        >
                          <td className="p-2">
                            <Checkbox
            ...
            <TabsContent value="matches">
              <div className="card-romantic space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <div>
                      ...
                      <Select
                        value={manualMatchMale}
                        onValueChange={setManualMatchMale}
                      >
                        <SelectTrigger>
                    ...
                    <div>
                      ...
                      <Select
                        value={manualMatchFemale}
                        onValueChange={setManualMatchFemale}
                      >
                        <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  {matches.map((m) => {
                    const male = users.find(
                      (u) => u.user_id === m.male_user_id,
                    );
                    const female = users.find(
                      (u) => u.user_id === m.female_user_id,
                    );
                    return (
                      <div
                        key={m.id}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-muted-foreground">
                            {m.is_instant_match ? "⚡ Instant" : "Regular"}
                          </span>
                          <Button
                            size="sm"
                            variant="destructive"
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <SelectContent>
                          {users
                            .filter(
                              (u) =>
                                !vipCodes.find(
                                  (v) => v.assigned_user_id === u.user_id,
                                ),
                            )
                            .map((u) => (
                              <SelectItem key={u.user_id} value={u.user_id}>
                    ...
                    <div>
                      ...
                      <Select
                        value={vipMatchWith}
                        onValueChange={setVipMatchWith}
                        disabled={!newVIPUser}
                      >
                        <SelectTrigger>
                          <SelectValue
                            placeholder={
                              newVIPUser ? "Select match" : "Select user first"
                            }
                          />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="flex items-end">
                      <Button
                        ...
                        disabled={creatingVIP || !newVIPUser || !vipMatchWith}
                        className="btn-romantic"
                      >
                        {creatingVIP ? (
                          <Loader2 className="w-4 h-4 animate-spin mr-2" />
                  ...
                  <p className="text-sm text-muted-foreground">
                    VIP codes allow instant match reveal. The match will be
                    created when the code is generated and revealed when the
                ...
                <div className="space-y-2">
                  ...
                  {vipCodes.map((v) => {
                    const user = users.find(
                      (u) => u.user_id === v.assigned_user_id,
                    );
                    const match = matches.find((m) => m.id === v.match_id);
                    let matchedWithUser = null;
                    if (match) {
                      const matchedUserId =
                        match.male_user_id === v.assigned_user_id
                          ? match.female_user_id
                          : match.male_user_id;
                      matchedWithUser = users.find(
                        (u) => u.user_id === matchedUserId,
                      );
                    }
                    return (
                      <div
                      ...
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full ${v.is_used ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                            >
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => deleteSponsor(inquiry.id)}
                        >
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      </div>
